...
function FileInput({ clearErrors, setValue, error, text, mountedPicture, name, required = true, register }) {
    ...
    return (
        <div className={'FileInput'}>
            ...
            <div className={`FileInput__Avatar ${error.hasOwnProperty(name) ? 'FileInput__Error' : ''}`} {...getRootProps()}>
                { files.length > 0
                    ? <button className={'FileInput__Delete'} onClick={(e) => {e.stopPropagation() ,setFiles([])}}>delete</button>
                    : null
                }
                { files.length !== 0
                        ? thumbs
